...
class DataLoader:
    ...
    def load_all(self):
        ...
        with open(segment_file, 'r') as f:
            self.segment_lengths = json.load(f)
    ...
    def validate_data(self):
        ...
        assert len(self.activations) == n_time_points, \
            "Activation data length mismatch"
        assert len(self.forces) == n_time_points, \
            "Force data length mismatch"
        ...
        assert set(act_muscles) == set(muscle_names), \
            "Muscle names mismatch between parameters and activations"
...
class PolytopeComputer:
    ...
    def compute_jacobian(self, hip_angle, knee_angle, ankle_angle,
        ...
        # This is approximate - full derivation would be more complex
        l1 = thigh_length
        l2 = shank_length
    ...
    def compute_polytope(self, moment_arms, muscle_forces, jacobian,
        ...
        # Sample muscle activation space (vertices of hypercube [0,1]^n)
...
class StairClimbingGUI:
    ...
    def create_gui(self):
        ...
        canvas = tk.Canvas(muscle_frame, height=250)
        ...
        self.ax_leg.set_xlim(-0.3, 0.5)
        self.ax_leg.set_ylim(-1.2, 0.2)
    ...
    def update_visualization(self):
        ...
        # Update labels
        progress = 100 * self.current_frame / (self.n_frames - 1)
        ...
        # Draw ground
        self.ax_leg.plot([-0.5, 0.8], [-1.0, -1.0], 'k-', linewidth=2)
        self.ax_leg.fill_between([-0.5, 0.8], [-1.0, -1.0], [-1.3, -1.3],
        ...
        for muscle in selected_muscles:
            if muscle in activations:
                ...
                if 'glut' in muscle or 'hip' in muscle:
                    # Hip muscles
        ...
        # Labels
        self.ax_leg.text(hip[0] - 0.1, hip[1], 'Hip', fontsize=9)
        self.ax_leg.text(knee[0] - 0.1, knee[1], 'Knee', fontsize=9)
        self.ax_leg.text(ankle[0] - 0.1, ankle[1], 'Ankle', fontsize=9)
        ...
        if len(selected_muscles) > 0:
            ...
            # Set limits based on polytope
            if len(vertices) > 0:
                margin = 200
                self.ax_polytope.set_xlim(vertices[:, 0].min() - margin,
                                          vertices[:, 0].max() + margin)
                self.ax_polytope.set_ylim(vertices[:, 1].min() - margin,
                                          vertices[:, 1].max() + margin)
...
def generate_sample_data():
    ...
    with open(data_path / "kinematics" / "segment_lengths.json", 'w') as f:
...
def main():
    ...
    if not Path("data").exists() or not Path("data/config.yaml").exists():
        print("Data directory not found. Generating sample data...")
